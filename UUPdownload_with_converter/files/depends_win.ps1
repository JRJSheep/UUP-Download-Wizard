...
function Retrieve-File {
    ...
    Write-Host -BackgroundColor Black -ForegroundColor Yellow "正在下载文件 ${File}……"
...
foreach($file in $files) {
    if(-not (Test-Hash -File $file -Hash $hashes[$file])) {
        Write-Error "文件 $file 似乎已被篡改"
        Exit 1
    }
}

Write-Host ""
Write-Host -BackgroundColor Black -ForegroundColor Green "似乎所有依赖项都已准备就绪。"
