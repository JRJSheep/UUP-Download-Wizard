...
set "WelDes=The settings on this page are used to control whether to enable revocation server detection. Selecting Default & echo Setting will enable revocation server detection; Select Alternate Setting will disable revocation server detection.& echo."
set "SelDes=This page is set to the website you used as the source to get the UUP file. Please select the appropriate server & echo from the list given in the menu according to your network conditions. Different servers may have different & echo requirements."
set "LSUUP=Local Server with UUP dump app"
...
set "LangMenu39=Update Only"
set "LangMenu40=Any Language"
...
set "ServerALL=Windows Server Standard Core, Standard, Datacenter Core, Datacenter, Windows Server Datacenter：Azure Edition, Windows Server Datacenter：Azure Edition Core"
...
set "SelDes=此页设置用于你从何网站做为来源获取 UUP 文件。请根据自己网络条件在菜单中给定的列表中选择合适的 & echo 服务器。不同服务器可能会有不同的要求。& echo."
set "LSUUP=本地服务器（使用 UUP dump app）"
set "Def=默认设置"
set "Alter=备用设置"
...
set "ServerALL=Windows Server Standard Core、Standard、Datacenter Core、Datacenter、Windows Server Datacenter：Azure Edition Core 和 Windows Server Datacenter：Azure Edition"
